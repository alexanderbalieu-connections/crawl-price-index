...
let topPrice = "$0.50";
const priceNums = prices.map(p => parseFloat((p.match(/(\d[\d.]*)/) || [])[1])).filter(n => !isNaN(n));
...
// (push-dataset.cjs) and served only through the gated API to active subs.
const paidDataset = {
  name: "The Crawl Price Index — full dataset",
  generated_utc: summary.generated_utc,
  cadence: "weekly",
  ...
  enforcement,
  tdmrep: summary.tdmrep || null,
  methodology_version: METHODOLOGY_VERSION,
  evidence: EVIDENCE_MODEL,
  corrections: readCorrections(),
  gtld_baseline: gtldBaseline,
};
// wide_probe: honest-identity scan of top-N + all blockers (additive key)
if (fs.existsSync("wide-probe.json")) {
  try { paidDataset.wide_probe = JSON.parse(fs.readFileSync("wide-probe.json", "utf8")); } catch (e) {}
}
fs.writeFileSync("paid-dataset.json", JSON.stringify(paidDataset));
// safety: make sure paid-dataset.json is gitignored so it can NEVER be pushed public
try {
  const gi = fs.existsSync(".gitignore") ? fs.readFileSync(".gitignore", "utf8") : "";
  if (!gi.includes("paid-dataset.json")) fs.appendFileSync(".gitignore", "\n# NEVER publish the paid dataset\npaid-dataset.json\n");
} catch (e) {}
...
// Runs archive.cjs (snapshot this scan into history/) then trends.cjs (deltas).
// Wrapped so a trends hiccup never breaks the core publish.
try {
  ...
  if (fs.existsSync("paid-dataset.json") && fs.existsSync("trends.json")) {
    ...
    paid.trends = JSON.parse(fs.readFileSync("trends.json", "utf8"));
    fs.writeFileSync("paid-dataset.json", JSON.stringify(paid));
    console.log("  trends folded into paid dataset");
  }
} catch (e) {
  console.log("  (archive/trends step skipped: " + e.message + ")");
}
