...
if (s.includes("function brandCard")) { console.log("already patched"); process.exit(0); }
...
if (![SL, S1, S2, W1, W2].every(x => s.includes(x))) { console.error("anchors missing — aborting"); process.exit(1); }
fs.writeFileSync("worker.js.bak6", s);
...
const newSub = `async function subscribe(request, env, cors) {
  ...
  if (!existing || existing.status !== "active") {
    ...
    const htm = brandCard('<p style="margin:0 0 6px"><b>One click and you are in.</b></p>'
      + '<p style="margin:0">Confirming gets you an <b>instant free data sample</b> — the top-100 domains&#39; complete AI-crawler rows — plus one email a week with the movers of the crawl economy.</p>'
      + btn(cUrl, 'Confirm subscription')
      + '<p style="font-size:11.5px;color:#6b787d;margin:0">If you did not request this, ignore this email.</p>');
...
const newWelcome = `  // welcome email: instant free sample link (non-fatal)
  try {
...
cat > patch-brand-emails.cjs << 'EOF'
const fs = require("fs");
let s = fs.readFileSync("worker.js", "utf8");
if (s.includes("function brandCard")) { console.log("already patched"); process.exit(0); }
const SL = "async function sendListEmail";
...
s = s.slice(0, w1) + newWelcome + s.slice(w2);

fs.writeFileSync("worker.js", s);
console.log("branded transactional emails (backup: worker.js.bak6)");
